...
def tt_nico_5p(filename):
    ...
    def fres(vs, vdat, sy, sts, std, lenx, ss, zi):  # Residuals array
        ...
        xf  = X0 + XP * zi
        yf  = Y0 + YP * zi
        tff = T0 + sc * kz * zi + xf * ss
        tbf = T0 + sc * kz * zi + (lenx - xf) * ss
        tsf = 0.5 * (tff + tbf)
        tdf = 0.5 * (tff - tbf)
        #  Data
        yd  = vdat[0]
    ...
    sts = 0.3    # t0 uncertainty
    std = 0.085  # tx uncertainty
    #
    #%%    
    npar = 5   #6
    nvar = 3
    ...
    vcts = []
    vctd = []
    vcsf = []  # sf: chi2 survival function
    #%%
    ...
    # ntrk = 1  # ===================================== Nb. of tracks to be fitted
    for it in range(ntrk):
        ...
        chsf = stats.chi2.sf(chi2r,ndf) # Survival function. ch2 goodness = 1 - F(chi2, ndf)
        vcsf.append(chsf)
        #print(mfit)
        #print('*',vs)
        vs = np.array(vs)
