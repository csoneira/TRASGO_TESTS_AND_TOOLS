...
def tt_nico(filename):
        ...
        if not_residuals == False:
            ...
            for i_plane in range(nplan):
                ...
                for ip in range(len(mdat_short)):   # loop on fired planes
                    ...
                    # deltax_tmp.append(v_res[3])
                    v_res_ip = v_track + v_res[0:3]
                    # print(v_res_ip)
                
                v_res_ip  = np.array(v_res_ip)
                
                v_res_ip = np.hstack( (v_res_ip, mdat[:,12]) )
                # print(v_res_ip)
                m_res_ip.append(v_res_ip)
                
                mDeltaY_both.append(mdat[:,12])
    ...
    #print('===', mfit)
    return mfit, vchi, vcsf, mcha, mcha_slew, m_res_ip, mDeltaY, mDeltaY_both