...
# -*- coding: utf-8 -*-
"""
Created on Thu Oct  3 14:13:02 2024
...
# Preparing the data as done earlier
sipm_signals_matrix = []
for row in sipm_signals:
    mat = row.reshape(num_sipms_row, num_sipms_row)
    sipm_signals_matrix.append(mat)
sipm_signals_matrix = np.array(sipm_signals_matrix)
sipm_signals_matrix = np.expand_dims(sipm_signals_matrix, axis=-1)  # Add the channel dimension

# Split into training and test sets
X_train, X_test, y_train, y_test = train_test_split(sipm_signals_matrix, real_positions, test_size=0.5, random_state=42)

# CNN model as already defined
input_shape = (num_sipms_row, num_sipms_row, 1)

best_model = Sequential([
    # Convolutional layers
    Conv2D(512, kernel_size=(3, 3), activation='relu', input_shape=input_shape, padding='same'),
    ...
    # Output layer (no activation for regression tasks like predicting positions)
    Dense(3)
])
...
def plot_residual_histograms(y_test, y_pred):
    ...
    for i, label in enumerate(['X', 'Y', 'Z']):
        ...
        axes[i].plot(x_values, gaussian_1d(x_values, *popt), 'r-', label=f'Gaussian fit\nsigma={popt[2]:.2f}')
...
# Function to plot residual contour for XY
...
# Full plotting function to call all plots
