...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct  2 10:25:49 2024
...
if hyperparameter_tuning:
    ...
    def build_model(hp):
        ...
        model.add(Dense(units=hp.Int('units_input', min_value=32, max_value=128, step=16),
                        activation='relu', input_shape=(num_sipms,),
                        kernel_regularizer=l2(0.001)))  # L2 regularization added
        ...
        for i in range(hp.Int('num_hidden_layers', 1, 3)):
            ...
            model.add(Dropout(rate=hp.Float(f'dropout_{i}', min_value=0.0, max_value=0.5, step=0.1)))  # Dropout added
        
        # Output Layer
        model.add(Dense(3))  # Predicting x, y, z
    ...
    # Setup a Keras Tuner Random Search
    tuner = RandomSearch(
        build_model,
        objective='val_loss',
        ...
        project_name='sipm_position_model_optimization'
    )
