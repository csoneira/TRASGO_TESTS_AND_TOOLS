...
# plt.plot(df_real_resampled.index, df_real_resampled / np.mean(df_real_resampled), label='Real Crossing Events', color='blue')
# plt.plot(df_crossing_resampled.index, df_crossing_resampled / np.mean(df_crossing_resampled), label='Detected Crossing Events', color='red')

real = df_real_resampled / (time_window_int*60) / (2*xlim) / (2*ylim) / 2 / np.pi * 100
plt.plot(df_real_resampled.index, real, label='Real Crossing Events', color='blue')

new_y = df_crossing_resampled / (time_window_int*60) / 30 / 30 / 2 / np.pi
plt.plot(df_crossing_resampled.index, new_y, label='Detected Crossing Events', color='red')
...
for time_window_int in minutes_list:
    ...
    # Align indices
    real, new_y = real.align(new_y, join='inner')

    # Calculate residuals
    residuals_real = real - new_y  # Real vs Detected
    residuals_flux = new_y - FLUX  # Detected vs FLUX
    
    residuals_real = residuals_real / real
    residuals_flux = residuals_real / FLUX
    
    # Ignore cases with fewer than 7 points
    if len(residuals_real) < 10:
        continue
...
# Plot residuals for real vs detected
plt.figure(figsize=(12, 6))
plt.errorbar(
    time_window_sizes,
    mean_residuals_real,
...
plt.title('Residuals of Real vs Detected Crossing Events')
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.savefig("residuals_real_vs_detected.png")

if show_plots:
    plt.show()

# Plot residuals for detected vs flux
plt.figure(figsize=(12, 6))
plt.errorbar(
    time_window_sizes,
    mean_residuals_flux,
    ...
    fmt='o',
    capsize=5,
    label='Residuals (Detected vs Flux)'
)
plt.xlabel('Time Window (minutes)')
plt.ylabel('Residuals')
plt.title('Residuals of Detected Events vs Flux')
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.savefig("residuals_detected_vs_flux.png")
